...
- **命令列表区**:
  ...
  - **空状态提示**: 当没有命令时，会根据上下文（是否在搜索）显示不同的提示信息和操作按钮（如"新建命令模板"或"清除筛选"）。
- **拖拽排序**:
  - 使用 `Sortable.js` 实现。
...
- **可视化编辑**:
  - 提供表单来编辑命令的所有属性。
  - 支持动态添加、删除和编辑 `命令级参数`、`选项` 和 `分隔符`。
- **参数类型系统**: 所有参数和选项支持四种类型：
  - `必选参数`: 必须提供值的参数
  - `可选参数`: 可以提供也可以不提供的参数（默认类型）
  - `不可选参数`: 禁止使用的参数（用于标记已废弃或不适用的参数）
  - `互斥选项`: 与其他选项互斥的选项（只能选择其中一个）
- **选项验证**: 实时检查选项配置并提供智能提示：
  - 检测无效选项类型并报错
  - 提醒缺少必选选项
  - 标识互斥选项冲突
  - 显示禁用选项状态
- **选项参数**: 每个选项可以配置是否需要参数值，并为选项参数单独设置类型。
- **校验**: 对命令名称、内容等进行基础校验。
...
- **参数分类器 (`parameterClassifier`)**:
  - 能够自动将参数分为 `必选`、`可选`、`不可选`（禁止使用）、`互斥`（互斥选项）。
  - 识别参数的级别（`命令级`, `选项级`, `全局`）。
- **参数迁移**: `parameterMigration` 工具自动将旧版本的 `required` 字段转换为新的 `type` 字段系统，严格执行四种参数类型规范。
